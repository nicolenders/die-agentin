  ...
  try {
    const [cats, auds, talkRows] = await Promise.all([
      ...
      db.taxonomy.findMany({
        where: { kind: "AUDIENCE" },
        orderBy: { sortOrder: "asc" },
        include: { _count: { select: { talkAudiences: true } } },
      }),
      db.talk.findMany({
    ...
    <section>
      ...
      <div className="card bracket" style={{ marginTop: 16 }}>
        ...
        <form action={createTalk} style={{ maxWidth: 560 }}>
          ...
          <CategoryMultiSelect name="audienceIds" options={audienceTags.map((a) => ({ id: a.id, name: a.nameDe }))} emptyHint="Optional — erst eine Zielgruppe anlegen (oben)." />
